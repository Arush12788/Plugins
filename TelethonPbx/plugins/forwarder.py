...
async def atarif(event):
    ...
    for _ in range(count):
        message = f"{victim_username} {random.choice(LLL_STR)}"
        await event.reply(message)

CmdHelp("atarif").add_command(
    "atarif", None, "Starts raid on a user by sending insults a specified number of times, each insult in a separate message."
).add()

AGA DUNGA SAARI GARMI NIKAL JAAYEGI",
...
async def raid(event):
    ...
    await event.delete()
    for _ in range(count):
        message = f"{victim_username} {random.choice(RAID_STR)}"
        await event.reply(message)

CmdHelp("raid").add_command(
    "raid", None, "Starts raid on a user by sending insults a specified number of times, each insult in a separate message."
).add()
